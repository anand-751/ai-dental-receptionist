...
# def parse_time_string_to_24h(time_str: str) -> Optional[str]:
#     """
...
#         try:
#             raw_answer = handle_rag_query(text)

#             if not raw_answer:
#                 base = "Mujhe iske baare mein exact information nahi mili."
#             else:
#                 base = raw_answer.strip()

#             if "call" in base.lower() and "160055" in base:
#                 base = "Clinic ki services ke baare mein main bata sakti hoon, lekin contact number ki information abhi available nahi hai."
...
#             return mirror_reply(
#                 text,
#                 "Maaf kijiye, mujhe abhi information retrieve karne mein problem aa rahi hai."
#             )
...
#         return mirror_reply(
#             text,
#             "Maaf kijiye, main theek se samajh nahi paayi. Aap appointment book karna chahte hain, reschedule karna chahte hain, ya kisi dental problem ke baare mein puchna chahte hain?"
#         )
...
"""
        ...
        prompt = f"""
You are a polite dental clinic receptionist speaking in clear, natural English.

TASK:
Convert the message below into natural spoken English. Keep the same meaning.

STRICT RULES:
- Reply ONLY in plain English — no Hindi words at all
- Tone: warm, friendly, professional — like a real receptionist on the phone
- Keep it short and conversational — 1 to 3 sentences max
- Do NOT say "ji", "haan", "theek hai", "namaste" or any Hindi/Hinglish words
- Convert times naturally: "09:30" → "9:30 in the morning", "13:00" → "1 in the afternoon"
- If the message mentions booking confirmation, sound happy and reassuring

MESSAGE:
{base_message}

Return ONLY the final spoken English sentence. No explanation, no extra text.
"""
...
def extract_time_from_text(text: str) -> Optional[str]:
    ...
    text = text.lower()
    text = text.replace("ke", " ").replace("ko", " ")
...
def resolve_slot_with_llm(user_text: str, slots: list) -> dict:
    """
    Asks LLM to pick one of the provided slots. Returns strict JSON.
    Normalizes returned time into 24h before using.
    """
    slots_text = "\n".join(
        f"- {s['date']} {s['time']}"
        for s in slots
...
async def _confirm_booking_on_calendar(session: dict, text: str = "", language: str = "hi") -> dict:
    """
    Called ONLY after payment is confirmed.
    Reads the saved slot from session["booking_data"]["selected_slot"]
    and calls book_slot() on Google Calendar.
    ...
    slot = session["booking_data"].get("selected_slot")

    if not slot:
        print("❌ _confirm_booking_on_calendar: no selected_slot in session")
        return mirror_reply(text, "Booking data nahi mili. Kripya phir se try karein.", language)
    ...
    try:
        ...
        session["payment_session_id"] = None

        base = f"Haan ji, aapki appointment confirm ho gayi hai: {date_str} ko {time_12h}."
        return {
            "type": "booking_confirmation",
            "message": mirror_reply(text, base, language),
            "booking": {
                "date": date_str,
                "time": time_12h,
                "event_id": event_id
        ...
        print(f"[_confirm_booking_on_calendar] exception: {e}")
        session["booking_state"] = None
        session["booking_data"].clear()
        return mirror_reply(text, "Maaf kijiye, booking karne mein dikkat aa gayi. Kripya thodi der baad try karein.", language)


# ======================= MAIN ROUTER =======================

async def route_intent(intent: str, text: str, session, language: str = "hi"):
    ...
    if intent == "greeting":
        base = "Hi, I'm the AI receptionist from Bright Dental Clinic. How may I assist you today?"
        return mirror_reply(text, base, language)
    ...
    if intent == "booking":
        ...
        if session["booking_state"] is None:
            if not refresh_token:
                return mirror_reply(text, "Sorry, appointment booking is currently unavailable.", language)
...
"""
        ...
        # ----------------------------------------------------------
        # STEP 2 — User selects a slot → SAVE it, set slot_selected
        #          Do NOT call book_slot() yet — payment comes first.
        # ----------------------------------------------------------
        if session["booking_state"] == "awaiting_slot":
            slots = session["booking_data"]["slots"]
            ...
            if extracted_time:
                print(f"[booking] extracted_time = {extracted_time}")
                slot = next((s for s in slots if s["time"] == extracted_time), None)
                if slot:
                    resolved = {"date": slot["date"], "time": slot["time"]}
                else:
                    print("[booking] no exact match; falling back to LLM resolver")
                    resolved = resolve_slot_with_llm(text, slots)
            else:
                resolved = resolve_slot_with_llm(text, slots)
            ...
            if not resolved or not resolved.get("date") or not resolved.get("time"):
                examples = ["1 baje dopahar", "shaam 4 baje", "dedh 1 baje", "11:30 AM"]
                base = f"Mujhe exact time samajh nahi aaya. Aap is tarah bol sakte ho: {', '.join(examples)}"
                return mirror_reply(text, base, language)

            # Final lookup against available slots
            slot = next(
                (s for s in slots if s["date"] == resolved["date"] and s["time"] == resolved["time"]),
                None
            )
            ...
            if not slot:
                def to_12h(t):
                    return datetime.strptime(t, "%H:%M").strftime("%I:%M %p").lstrip("0")
                available_times = ", ".join([to_12h(s["time"]) for s in slots])
                base = f"Woh time available nahi hai. Available times hain: {available_times}. Inme se koi ek time bol dijiye."
                return mirror_reply(text, base, language)

            # ✅ SAVE selected slot — DO NOT book yet
            session["booking_data"]["selected_slot"] = slot

            # ✅ SET booking_state = "slot_selected"
            # conversation_stream.py watches for this state to trigger payment
            session["booking_state"] = "slot_selected"

            print(f"[booking] Slot selected and saved: {slot}. Awaiting payment.")

            time_12h = datetime.strptime(slot["time"], "%H:%M").strftime("%I:%M %p").lstrip("0")
            date_str = datetime.strptime(slot["date"], "%Y-%m-%d").strftime("%B %d, %Y")

            # Return a confirmation message — conversation_stream.py will
            # intercept this response and append payment_required signal
            base = f"Maine {date_str} ko {time_12h} ka slot select kar liya hai. Ab payment complete karein to appointment confirm ho jayegi."
            return mirror_reply(text, base, language)

        # ----------------------------------------------------------
        # STEP 3 — Payment confirmed → actually book on calendar
        # Called from conversation_stream.py after verifying payment
        # ----------------------------------------------------------
        if session["booking_state"] == "confirm_booking":
            print("[booking] Payment confirmed — proceeding to book on calendar")
            return await _confirm_booking_on_calendar(session, text, language)
    ...
    if intent == "reschedule":
        ...
        if not refresh_token:
            return mirror_reply(text, "Sorry, rescheduling is currently unavailable.", language)
        ...
        if session["reschedule_state"] is None:
            ...
            slots_text = ", ".join([to_12h(s['time']) for s in filtered[:6]])
            old_time_12h = datetime.strptime(event['time'], '%H:%M').strftime('%I:%M %p').lstrip('0')
            base = f"Maine aapki purani appointment payi: {event['date']} ko {old_time_12h}. Naye available times: {slots_text}. Aap kaunsa naya time chahenge?"
            return mirror_reply(text, base, language)
        ...
        if session["reschedule_state"] == "awaiting_new_slot":
            ...
            if not resolved or not resolved.get("date") or not resolved.get("time"):
                return mirror_reply(text, "Mujhe samajh nahi aaya kaunsa naya time chahiye. Kripya seedhe '3 PM' ya '11:30 AM' bolein.", language)
            ...
            if not slot:
                return mirror_reply(text, "Woh slot ab available nahi hai. Kripya koi aur choose karein.", language)
            ...
            try:
                ...
                print(f"[reschedule] exception: {e}")
                return mirror_reply(text, f"Reschedule failed: {str(e, language)}")
